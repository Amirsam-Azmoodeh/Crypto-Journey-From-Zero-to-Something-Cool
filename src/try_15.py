"""
try15: HKDF + Blake2s Keystream
--------------------------------
Concept: HKDF instead of PBKDF2, Blake2s for keystream
Security: MEDIUM - Better key derivation, still experimental
...
class ASA_Crypt:
    ...
    def create_state(self, salt: bytes, soft_state: bytes) -> bytes:
        ...
        R1 = hmac.new(self.key.encode(), kdf_salt + b'ASA-EXTRACT-1', hashlib.sha256).digest()
        R2 = hmac.new(R1, salt + b'ASA-EXPAND-1', hashlib.sha256).digest()
        R3 = hmac.new(R2, soft_state + b'ASA-MIX-1', hashlib.sha256).digest()
        state = hmac.new(R3, R1 + b'ASA-STREAM-1' + R2, hashlib.sha256).digest()
