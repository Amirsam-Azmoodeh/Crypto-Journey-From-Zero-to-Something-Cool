"""
try17: Fusion + Rotation (Improved)
------------------------------------
Concept: Better rotation parameters (16 bits instead of 5/13)
Security: MEDIUM - Still experimental
...
class ASA_Crypt:
    ...
    def fusion(self, salt, nonce, soft_state):
        rotated_salt = self.rotate_bits(salt, 16)
        rotated_nonce = self.rotate_bits(nonce, 16)
    ...
    def create_keystream(self, state: bytes, extra_string: bytes, nonce: bytes, block_counter: int) -> bytes:
        mixed = self.fusion(extra_string[0:16], nonce, extra_string[-16:])
        data = b'ASA-V1' + mixed + block_counter.to_bytes(8, 'big')
