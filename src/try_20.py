"""
try20: Set with Counter (Limited)
----------------------------------
Concept: Set with size limit to prevent memory overflow
Security: MEDIUM-HIGH - Limited set may miss replays
...
class ASA_Crypt:
    ...
    def create_keystream(self, state: bytes, nonce: bytes, block_counter: int) -> bytes:
        ...
        return hashlib.blake2s(data, key=state + b'ENC').digest()
