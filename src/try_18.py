"""
try18: Set-based Replay Protection (Buggy)
-------------------------------------------
Concept: Added set to prevent replay attacks
Security: MEDIUM - But has bug (using {} instead of set())
...
class ASA_Crypt:
    ...
    def create_mac_key(self, state: bytes) -> bytes:
        return hmac.new(state, b'ASA-MAC-1', hashlib.sha256).digest()
    ...
    def decrypt(self, full_message: str) -> str:
        ...
        if len(full_message) < 84:
            return

        if full_message in self.ciphertext_set:
            return

        plaintext = bytearray()

        salt = full_message[:16]
        nonce = full_message[16:40]
        header2 = full_message[40:52]
        ciphertext = full_message[52:-32]
        tag = full_message[-32:]
        ...
        self.ciphertext_set.add(full_message)
